...
`))
    ...
    Example:    /select 1 2 3 意味着选择 1 2 3 三个方法
    ...
    s
    select
    /select         选择方法        /select <id1> [id2] [id3]
    ...
    allocation
    /alloc
    /allocation     分配参数      /allocation <TAG> <[+/-]value>
        ...
        for(const number of select) {
            ...
            if(exclusive != null)
                for(const { name, id } of this.#talentSelected)
                    if(id == exclusive)
                        return warn(`方法【${s.name}】与已选择的方法【${name}】冲突`);
    ...
    next(enter) {
        ...
        switch(this.#step) {
            case this.Steps.TALENT:
                if(this.#talentSelected.size != 3) return warn(this.list(), `⚠请选择3个方法`);
            ...
            case this.Steps.PROPERTY:
                ...
                if(less > 0) return warn(this.prop(), `你还有${less}参数没有分配完`);
        ...
        switch(tag) {
            ...
            case 'MNY':
                ...
                if(
                    ...
                    || tempSet < 0
                ) return  warn('⚠ 不能分配负数参数');
                if(tempSet>10) return  warn('⚠ 单项参数最高分配10点');
