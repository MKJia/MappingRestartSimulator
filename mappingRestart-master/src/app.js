...
class App{
    ...
    initPages() {
        ...
        const indexPage = $(`
        <div id="main">
            <button id="achievement">增量建图</button>
            <button id="specialthanks">闭环修正</button>
            <button id="themeToggleBtn">黑</button>
            <div id="title">
                建图重开模拟器<br>
                <div style="font-size:1.5rem; font-weight:normal;">这垃圾建图软件一秒也不想用了</div>
            </div>
            <button id="restart" class="mainbtn"><span class="iconfont">&#xe6a7;</span>开始建图</button>
                    ...
                    .forEach(talent=>{
                        ...
                        li.click(()=>{
                            if(li.hasClass('selected')) {
                                ...
                                if(this.#talentSelected.size==3) {
                                    talentPage.find('#next').text('开始新建图')
                                }
                            }
                        });
                    });
        ...
        <div id="main">
            <div class="head" style="font-size: 1.6rem">
                <div>调整初始参数</div>
                <div id="total" style="font-size:1rem; font-weight:normal;">参数表：</div>
            </div>
            <ul id="propertyAllocation" class="propinitial"></ul>
            <ul class="selectlist" id="talentSelectedView"></ul>
            <div class="btn-area">
                <button id="random" class="mainbtn">随机参数</button>
                <button id="start" class="mainbtn">开始新建图</button>
            </div>
        </div>
        `);
        ...
        const freshTotal = ()=>{
            propertyPage.find('#total').text(`可用参数点：${this.#totalMax - total()}`);
        }
        ...
        groups.CHR = getBtnGroups("体素大小", 0, 10); // 建图速度 charm CHR
        groups.INT = getBtnGroups("网格大小", 0, 10); // 建图精度 intelligence INT
        groups.STR = getBtnGroups("迭代次数", 0, 10); // 建图完整度 strength STR
        groups.MNY = getBtnGroups("收敛条件", 0, 10); // 建图稠密度 money MNY
        ...
        propertyPage
            ...
            .click(()=>{
                if(total() < this.#totalMax) {
                    this.hint(`你还有${this.#totalMax-total()}参数点没有分配完`);
                    return;
                } else if (total() > this.#totalMax) {
                    this.hint(`你多使用了${total() - this.#totalMax}参数点`);
                    return;
                }
        ...
        <div id="main">
            ...
            <div class="btn-area">
                <button id="auto" class="mainbtn">自动建图</button>
                <button id="auto2x" class="mainbtn">自动建图2x</button>
                <button id="summary" class="mainbtn">建图总结</button>
                <button id="domToImage" class="mainbtn">重新建图</button>
            </div>
        ...
        trajectoryPage
            ...
            .click(()=>{
                ...
                const li = $(`<li><span>${age}个pcd：</span><span>${
                    content.map(
                ...
                $("#lifeProperty").html(`
                <li><span>建图速度</span><span>${property.CHR}</span></li>
                <li><span>建图精度</span><span>${property.INT}</span></li>
                <li><span>建图完整度</span><span>${property.STR}</span></li>
                <li><span>建图稠密度</span><span>${property.MNY}</span></li>
                <li><span>建图小熊度</span><span>${property.SPR}</span></li>
                `);
            });
        // html2canvas
        trajectoryPage
        ...
        const summaryPage = $(`
        <div id="main">
            <div class="head">建图总结</div>
            <ul id="judge" class="judge">
                <li class="grade2"><span>建图速度：</span><span>9级 美若天仙</span></li>
                <li class="grade0"><span>建图精度：</span><span>4级 建图精度一般</span></li>
                <li class="grade0"><span>建图完整度：</span><span>1级 极度虚弱</span></li>
                <li class="grade0"><span>建图稠密度：</span><span>6级 小康之家</span></li>
                <li class="grade0"><span>建图美观度：</span><span>3个pcd 早夭</span></li>
                <li class="grade0"><span>建图小熊度：</span><span></span>3级 不太幸福的人生</li>
            </ul>
            <div class="head" style="height:auto;">方法，你可以选一个，下辈子还能抽到</div>
            ...
            achievement: {
                ...
                clear: () => {
                    ...
                    total.append(`
                        ...
                        <li class="achvg${getGrade('talentRate', talentRate)}"><span class="achievementtitle">方法收集率</span>${Math.floor(talentRate * 100)}%</li>
                    `);
                    ...
                    if(contents.length > 0)
                        $('#lifeTrajectory')
                            .append(`<li><span>初始：</span><span>${
                                contents.map(
                                    ({source, target}) => `方法【${source.name}】发动：替换为方法【${target.name}】`
                                ).join('<br>')
                            }</span></li>`);
